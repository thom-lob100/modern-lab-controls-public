...
namespace Modern.Lab.Samples
{
    ...
    public partial class DurableManagementForm : ModernFormBase
    {
        ...
        private readonly NewItemTracker historyNewItems = new NewItemTracker();
        private readonly NewItemTracker stubHistoryNewItems = new NewItemTracker();
        private readonly NewItemTracker lccHistoryNewItems = new NewItemTracker();
        ...
        private void LoadDependents(string durableId, bool silent)
        {
            ...
            this.LoadDependent(channelHistory, () => this.RequestHistory(durableId),
                    table => this.BindHistory(table, NewItemTracker.Criteria(durableId)), silent, this.historyCover);
            ...
            this.LoadDependent(
                    ...
                    table => this.BindStubHistory(table, NewItemTracker.Criteria(durableId)), silent, this.historyCover);
            this.LoadDependent(
                    channelLccHistory,
                    () => this.RequestWaferHistory(durableId, ServerFields.Lot.SubProdTyp.Lamella),
                    table => this.BindLccHistory(table, NewItemTracker.Criteria(durableId)), silent, this.historyCover);
        }
        ...
        private void BindHistory(DataTable history, string criteria)
        {
            this.historyData = this.historyNewItems.Accept(history, this.gridHistory.RowKeyMember, criteria);

            DurableHistoryColumns(this.historyData).Bind(this.gridHistory);
            NewItemTint.Bind(this.gridHistory, this.historyNewItems, this.gridHistory.RowKeyMember, null, false);
        }

        private void BindStubHistory(DataTable history, string criteria)
        {
            this.stubHistoryData = DurableManagementPresenter.MarkJudgeColors(
                    this.stubHistoryNewItems.Accept(history, this.gridStubHistory.RowKeyMember, criteria));
            this.BindWaferHistory(this.gridStubHistory, this.stubHistoryData, "Chip Id", this.stubHistoryNewItems);
        }

        private void BindLccHistory(DataTable history, string criteria)
        {
            this.lccHistoryData = DurableManagementPresenter.MarkJudgeColors(
                    this.lccHistoryNewItems.Accept(history, this.gridLccHistory.RowKeyMember, criteria));
            this.BindWaferHistory(this.gridLccHistory, this.lccHistoryData, "Lamella Id", this.lccHistoryNewItems);
        }

        private void BindWaferHistory(
                Modern.Lab.WinForms.Controls.Data.ModernDataGrid grid, DataTable history, string idCaption, NewItemTracker tracker)
        {
            WaferHistoryColumns(history, idCaption).Bind(grid);
            NewItemTint.Bind(grid, tracker, grid.RowKeyMember, null, false);
        }
