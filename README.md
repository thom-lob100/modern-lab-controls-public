...
- 범위: 기존 반영 대기 22개에 쓰기 중 잠금 관련 파일을 합친 최신 소스 25개. 2026-09-23 16:30 KST는 사용자가 알려준 잠정 기준이며, 회사 사본과 직접 비교한 결과는 아니다.
- 원본 기준 커밋: `204e6022d7a23ce20bf772b667cf62860a9f465a` (`main`). 쓰기 중 잠금 구현 `2007366b`와 Codex 검토 기록까지 원본에 커밋·푸시되어 있으며, 아래 소스 25개는 그 원본과 일치한다.
- 이번 전달은 6개 소스 변경이다. Public에 새로 추가한 파일은 `ModernFormBase.cs`·`MasterDataCrudView.cs`·`UserForm.cs`, 최신본으로 교체한 파일은 `MasterDataCrudFormBase.cs`·`ReasonCodeForm.cs`·`CommonCodeForm.cs`다. 기존 대기분도 모두 포함한다.
...
| 상위 Common | [Modern.Lab.Hosting/MasterData/MasterDataCrudFormBase.cs](Modern.Lab.Hosting/MasterData/MasterDataCrudFormBase.cs) | 복합 키 CRUD·신규 입력·재선택·쓰기 중 입력 잠금 |
...
| 마스터데이터 | [Modern.Lab.MasterData/Screens/CommonCodes/CommonCodeForm.cs](Modern.Lab.MasterData/Screens/CommonCodes/CommonCodeForm.cs) | 공통 CRUD·기존 키 원값 재선택·중복 잠금 제거, 대분류 입력은 폼에서 잠금 |
...
| 전달분 | 회사 반영 상태 |
|---|---|
| 2026-09-25 / 위 25개 파일 | 반영 대기 — 아직 완료 확인을 받지 않음 |
