...
namespace Modern.Lab.MasterData
{
    public partial class PfoNodeForm : ModernFormBase
    {
        ...
        private static readonly TableContract NodeContract = new TableContract("PfoNode.SelectPfoNodes.Node")
                .Key(NodeIdColumn)
                .Require(FromNodeIdColumn, ToNodeIdColumn,
                        NodeSequenceColumn, ProductProcessColumn,
                        FlowProcessColumn, OperProcessColumn);
        ...
        public PfoNodeForm()
        {
            ...
            this.cboProduct.DisplayMember = ProductIdColumn;
        ...
        {
            DataTable products = this.SelectPfoProducts();
            DataTable flows = this.SelectPfoFlows();
            DataTable opers = this.SelectPfoOpers();
        ...
        private void OnCreateProductNodeClick(object sender, EventArgs e)
        {
            ...
            this.RunWrite(
                    () => this.EnsureProductNode(productId),
        ...
        private void OnCancelClick(object sender, EventArgs e)
        {
            ...
            this.BuildTree(this.loadedNodeTable);
            this.screenState = this.treeNodes.Nodes.Count == 0 ? ScreenState.MissingRoot : ScreenState.Ready;
            this.HideNotice();
            this.UpdateActionState();
        }
        ...
        private void OnSaveClick(object sender, EventArgs e)
        {
            ...
            string productId = this.SelectedProductId();
            this.RunWrite(
                    () => this.SavePfoNodes(productId, plan, selectedClientId),
        ...
        private sealed class NodeItem
        {
            ...
            {
                string product = Text(row, ProductProcessColumn);
                string flow = Text(row, FlowProcessColumn);
                string oper = Text(row, OperProcessColumn);
                NodeKind kind = product.Length > 0 ? NodeKind.Product : (flow.Length > 0 ? NodeKind.Flow : NodeKind.Oper);
                int sequence;
                int.TryParse(Text(row, NodeSequenceColumn), NumberStyles.Integer,
                ...
                return new NodeItem
                {
                    NodeId = Text(row, NodeIdColumn),
                    FromNodeId = Text(row, FromNodeIdColumn),
